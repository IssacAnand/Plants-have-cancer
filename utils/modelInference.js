...
export async function loadModel() {
  try {
    ...
    [imageSession, textSession, mlpSession] = await Promise.all([
    ...
    console.log("[model] All three ONNX sessions loaded");
    return true;
  } catch (err) {
...
// ── Heatmap generation (gradient-free mean-CAM) ───────────────────────────────
...
export async function analyzeLeaf(imageUri, symptomText = "") {
  if (!imageSession || !textSession || !mlpSession) {
  ...
  try {
    heatmapUri = buildHeatmap(
      spatialFeat.data,
      spatialFeat.dims,
    );
  } catch (err) {
    console.warn("[model] Heatmap generation failed:", err);
  }
